...
public sealed class TeamsController(AisamContext db, AccessScope scope) : ControllerBase
{
    ...
    {
        if (!scope.IsOwner) return Forbid();
        RegisterOwnerMutationAuthorization($"Update:{id}");
    ...
    {
        if (!scope.IsOwner) return Forbid();
        RegisterOwnerMutationAuthorization($"Delete:{id}");
    ...
    {
        if (!scope.IsOwner) return Forbid();
        RegisterOwnerMutationAuthorization($"SetAccess:{teamId}:{brandId}");
    ...
    {
        if (!scope.IsOwner) return Forbid();
        RegisterOwnerMutationAuthorization($"Transfer:{teamId}:{userId}:{request.TargetTeamId}");
    ...
    private void RegisterOwnerMutationAuthorization(string operation)
    {
        db.RegisterMutationAuthorization(
            scope.WorkspaceId,
            $"Team:{operation}",
            scope.PermissionRevision,
            token => db.WorkspaceMembers
                .AsNoTracking()
                .AnyAsync(member =>
