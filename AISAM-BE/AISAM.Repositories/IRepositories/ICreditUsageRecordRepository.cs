...
public interface ICreditUsageRecordRepository
{
    ...
    Task<PagedResult<CreditUsageRecord>> GetPagedByWorkspaceAndUserIdAsync(Guid workspaceId, Guid userId, PaginationRequest request, CancellationToken cancellationToken = default);
    ...
    Task<int> GetTotalCreditsUsedAsync(CancellationToken cancellationToken = default);
    Task<IReadOnlyList<CreditUsageRecord>> GetAllAsync(CancellationToken cancellationToken = default);
}
