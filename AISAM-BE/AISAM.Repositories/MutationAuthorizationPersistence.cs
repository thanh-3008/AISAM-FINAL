...
{
    private sealed record MutationStamp(
        Guid WorkspaceId,
        long Revision,
        Func<CancellationToken, Task<bool>> Revalidate,
        DateTime AuthorizedAt,
        bool RevalidateAfterWrite);
    private readonly Dictionary<string, MutationStamp> mutationStamps = new();

    public void RegisterMutationAuthorization(
        Guid workspace,
        string key,
        long revision,
        Func<CancellationToken, Task<bool>> revalidate,
        bool revalidateAfterWrite = true) =>
        mutationStamps[key] = new(workspace, revision, revalidate, DateTime.UtcNow, revalidateAfterWrite);
    ...
    private async Task<int> SaveWithMutationGuardAsync(bool acceptAllChanges, CancellationToken ct)
    {
        ...
        {
            await ValidateMutationStampsAsync(ct, afterWrite: false);
        ...
        return await Database.CreateExecutionStrategy().ExecuteAsync(async () =>
        {
            ...
            try
            {
                ...
                await ValidateMutationStampsAsync(ct, afterWrite: true);
    ...
    private async Task ValidateMutationStampsAsync(CancellationToken ct, bool afterWrite)
    {
        foreach (var stamp in mutationStamps.Values.ToArray())
        {
            if (afterWrite && !stamp.RevalidateAfterWrite) continue;
