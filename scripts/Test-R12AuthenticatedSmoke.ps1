param(
    [string]$ApiAssembly = 'bin/Debug/net8.0/AISAM.API.dll',
    [int]$Port = 5128,
    [switch]$VerifyHrWrite
)
...
$apiWorkingDirectory = Join-Path $root 'AISAM-BE/AISAM.API'
$apiArgument = $ApiAssembly
if ([IO.Path]::IsPathRooted($ApiAssembly)) {
    $apiWorkingDirectory = Split-Path $ApiAssembly -Parent
    $apiArgument = Split-Path $ApiAssembly -Leaf
}
$baseUrl = "http://127.0.0.1:$Port"
$process = Start-Process -FilePath "$env:DOTNET_ROOT/dotnet.exe" -ArgumentList @($apiArgument,'--urls',$baseUrl) -WorkingDirectory $apiWorkingDirectory -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $out 'authenticated-api.log') -RedirectStandardError (Join-Path $out 'authenticated-api-error.log')
$results = @()
try {
    $ready = $false
    for ($i=0;$i -lt 30;$i++) {
        try { Invoke-WebRequest "$baseUrl/api/health" -UseBasicParsing -TimeoutSec 2 | Out-Null; $ready=$true; break } catch { Start-Sleep -Milliseconds 300 }
    }
    ...
    foreach ($actor in $actors) {
        ...
        $response=Invoke-RestMethod "$baseUrl/api/permissions/context" -Headers $headers
        ...
        $status=0
        try { Invoke-WebRequest "$baseUrl/api/permissions/context" -Headers $headers -UseBasicParsing | Out-Null }
        ...
        if ($VerifyHrWrite -and $expected -eq 'Owner') {
            $headers['X-Workspace-Id']=$parts[1]
            $teamList=Invoke-WebRequest "$baseUrl/api/teams/manage" -Headers $headers -UseBasicParsing
            $headers['If-Match']=$teamList.Headers['X-HR-Revision']
            $payload=@{name='RBAC transaction smoke '+[Guid]::NewGuid().ToString('N').Substring(0,8);description='temporary verification';members=@()} | ConvertTo-Json -Compress
            $created=Invoke-WebRequest "$baseUrl/api/teams" -Method POST -Headers $headers -ContentType 'application/json' -Body $payload -UseBasicParsing
            if ($created.StatusCode -ne 201) { throw "HR create failed: $($created.StatusCode)" }
            $teamId=(ConvertFrom-Json $created.Content).data.id
            $headers['If-Match']=$created.Headers['X-HR-Revision']
            $deleted=Invoke-WebRequest "$baseUrl/api/teams/$teamId" -Method DELETE -Headers $headers -UseBasicParsing
            if ($deleted.StatusCode -ne 200) { throw "HR cleanup failed: $($deleted.StatusCode)" }
            $results += @{case='PostgreSQL retrying HR transaction';role=$expected;create=201;cleanup=200;result='PASS'}
        }
    }
