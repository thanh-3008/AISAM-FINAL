...
| CT-03 | Tạo content VideoText với upload video | 1. Đăng nhập test@example.com / Pass1234 2. Content -> Create New Content -> Manual Creation 3. Title: "Hướng dẫn sử dụng sản phẩm" 4. Chọn Brand: "My Brand", Product: "Áo Thun Cotton" 5. Content Type: click "Video" 6. Click "Click to upload" trong vùng Upload Video 7. Chọn file tutorial.mp4 (30MB, 30 giây) 8. Quan sát tên file và preview video với controls play/pause ở cả Upload Video và Post Preview 9. Nhập Duration: "0:30" 10. Nhập Caption: "Xem ngay cách phối đồ với áo thun!" 11. Status: Draft 12. Click Save Content | Toast hiển thị **"Content created successfully"**. Tên gốc `tutorial.mp4` hiển thị và không mất khi đổi tab preview. Card hiển thị thumbnail video + icon play. Type badge VIDEO (hồng). Detail có video player. API payload adType=2 | File tutorial.mp4 <=50MB |
...
| CT-13 | Remove ảnh đã upload | 1. Đăng nhập -> Content -> Create -> Type IMAGE 2. Upload ảnh bất kỳ 3. Click nút X (Close) trên ảnh 4. Quan sát vùng upload | Vùng upload về trạng thái ban đầu: icon add_photo_alternate, text "Click to upload" / "or drag and drop your image here" / "PNG, JPG, WebP, GIF up to 50MB". Ảnh cũ bị xóa. Khi Save -> imageUrl = undefined | Đã upload ảnh |
| CT-14 | Upload ảnh hợp lệ 12MB | 1. Đăng nhập -> Content -> Create -> Type IMAGE 2. Upload file photo.jpg (12MB) 3. Nhập Title + Brand + Product 4. Click Save Content | Label và validation thống nhất giới hạn 50MB. Upload thành công vì 12MB < 50MB. Toast hiển thị **"Content created successfully"** | File 12MB |
| CT-15 | Upload ảnh >50MB (vượt code limit) | 1. Đăng nhập -> Content -> Create -> Type IMAGE 2. Chọn file photo-big.jpg (51MB) 3. Quan sát form | Frontend từ chối ngay trước upload và hiển thị **"The selected image is too large. Maximum allowed size is 50 MB."** File không được chọn, request `/content/media` không được gửi. Backend vẫn độc lập từ chối file >50MB nếu request vượt qua frontend | File 51MB |
