...
it("persists an attempt before timeout and reopens without submitting again", async () => {
  ...
  await screen.findByRole("button", { name: "Check results" });
  expect(screen.queryByRole("button", { name: /Publish to/ })).toBeNull();
  fireEvent.click(screen.getByRole("button", { name: "Check results" }));
...
it("resumes a server-missing attempt with the same idempotency key", async () => {
  ...
  render(<PostNowModal {...props} />);
  fireEvent.click(await screen.findByRole("button", { name: "Check results" }));
  fireEvent.click(await screen.findByRole("button", { name: "Continue publish request" }));
...
it("prepares only the failed destination after partial publication", async () => {
  ...
  const retry = await screen.findByRole("button", { name: /Publish to 1/ });
