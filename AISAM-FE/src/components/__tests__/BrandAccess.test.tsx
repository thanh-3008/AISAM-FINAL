...
it("requires view before publish and sends the current revision", async () => {
  ...
  expect((screen.getByRole("checkbox", { name: "Publish" }) as HTMLInputElement).disabled).toBe(true);
  ...
  await waitFor(() => expect((screen.getByRole("checkbox", { name: "Publish" }) as HTMLInputElement).disabled).toBe(false));
});
it("shows a conflict and removes stale controls until explicit reload", async () => {
  vi.mocked(changeAssignment).mockRejectedValue(Object.assign(new Error("Permissions have changed. Reload."), { status: 409 }));
  render(<BrandAccessPage />);
  fireEvent.click(await screen.findByRole("checkbox", { name: "View" }));
  expect((await screen.findByRole("alert")).textContent).toContain("Reload");
  expect(screen.queryByRole("checkbox", { name: "View" })).toBeNull();
