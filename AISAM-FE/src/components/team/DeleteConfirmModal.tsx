...
export default function DeleteConfirmModal({ teams, isLoading, onConfirm, onCancel }: DeleteConfirmModalProps) {
  if (teams.length === 0) return null;

  const isBulk = teams.length > 1;
          ...
          <div className="p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-headline-sm text-on-surface font-semibold">
                  {isBulk ? `Delete ${teams.length} Teams` : "Delete Team"}
                </h3>
            ...
            {!isBulk && (
              <p className="text-body-sm text-on-surface-variant mb-4">
                Are you sure you want to delete <span className="font-semibold text-on-surface">{teams[0].name}</span>?
              </p>
            )}

            {isBulk && (
              <div className="mb-4 p-3 bg-surface-container-low rounded-xl max-h-32 overflow-y-auto">
                <p className="text-label-xs text-outline font-semibold mb-2">Teams to delete:</p>
                <ul className="space-y-1">
                  {teams.map((t) => (
                    <li key={t.id} className="text-label-xs text-on-surface flex items-center gap-2">
            ...
            <p className="text-label-xs text-outline mb-6">
              All team data and member assignments will be permanently removed.
            </p>
