          ...
          <div className="space-y-4 animate-fade-up">
            {aiResponse.summary && (
              <div className="rounded-xl border border-indigo-200 bg-indigo-50/70 p-4">
                <p className="text-xs font-bold uppercase tracking-wider text-indigo-600 mb-1">Overview</p>
                ...
                <div className="space-y-3">
                  {aiResponse.next_post_actions!.map((item, index) => (
                    <div key={`${item.action}-${index}`} className="rounded-lg bg-white p-3 shadow-sm ring-1 ring-blue-100">
                      ...
                      {item.kpi_target && <p className="mt-2 flex items-center gap-1.5 text-xs font-semibold text-blue-700"><span className="material-symbols-outlined text-[15px]">track_changes</span>Target: {item.kpi_target}</p>}
                    </div>
                  ))}
                </div>
              </section>
      ...
      {showAiResults && (
        <div className="mt-4 p-3 bg-surface-container-low/50 rounded-lg border border-outline-variant/30 flex items-start gap-2 relative z-10 animate-fade-up" style={{ animationDelay: "1s" }}>
          ...
          <p className="text-xs text-on-surface-variant italic leading-relaxed">
            Note: The above recommendations are generated by AI for reference purposes. Please consider carefully before making key marketing decisions.
          </p>
        </div>
      )}
