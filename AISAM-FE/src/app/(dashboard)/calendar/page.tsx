...
import {
  fetchSchedules, bulkCreateSchedules, createSchedule,
...
function CalendarContent() {
  ...
  const rbac = useRbac();
  const canManageSchedules = rbac ? rbac.workspaceRole !== "Member" || rbac.teams.some(t => t.role === "Manager") : featureGate.can("manageSchedules");
  ...
  const publishAllowed = useResourcePermissions(rbac && form.contentId ? integrations.map(i => ({ kind: Kind.Content, resourceId: form.contentId, channelId: i.id, permission: Permission.PostPublish })) : []);
  const canUseChannel = (id: string) => !rbac || publishAllowed(integrations.findIndex(i => i.id === id));
  const scheduleAllowed = useResourcePermissions(rbac ? schedules.map(s => ({ kind: Kind.Content, resourceId: s.contentId, channelId: s.integrationId, permission: Permission.PostPublish })) : []);
  const editAllowed = useResourcePermissions(rbac && editingSchedule ? integrations.map(i => ({ kind: Kind.Content, resourceId: editingSchedule.contentId, channelId: i.id, permission: Permission.PostPublish })) : []);
  const canEditChannel = (id: string) => !rbac || editAllowed(integrations.findIndex(i => i.id === id));
  const canModifySchedule = (id: string) => canManageSchedules && (!rbac || scheduleAllowed(schedules.findIndex(s => s.id === id))) && schedules.some(s => s.id === id && s.status !== "Completed" && s.status !== "Processing");
  ...
  const handleCreate = async () => {
    ...
    if (createIntegrationIds.some(id => !canUseChannel(id))) { setToast("Bạn không có quyền lên lịch nội dung này trên kênh đã chọn."); return; }
  ...
  const handleDelete = async (id: string) => {
    if (!canModifySchedule(id)) { setToast("Bạn không có quyền sửa lịch này hoặc lịch đang được xử lý."); return; }
    setActionId(id);
  ...
  const handleUndoDelete = async () => {
    if (!deletedItem || actionId) return;
    setActionId("restore");
    try {
      if (rbac) {
        const [allowed] = await checkPermissions([{ kind: Kind.Content, resourceId: deletedItem.contentId, channelId: deletedItem.integrationId, permission: Permission.PostPublish }]);
        if (!allowed) throw new Error("Bạn không còn quyền tạo lại lịch này.");
      }
  ...
  const openEditModal = (s: ScheduleItem) => {
    if (!canModifySchedule(s.id)) { setToast("Bạn không có quyền sửa lịch này hoặc lịch đang được xử lý."); return; }
  ...
  const handleEditSave = async () => {
    ...
    if (!canModifySchedule(editingSchedule.id) || !canEditChannel(editForm.integrationId)) { setToast("Bạn không có quyền cập nhật nội dung trên kênh đã chọn."); return; }
    setActionId("edit");
                          ...
                          return (
                            <div key={s.id} className="group p-3 rounded-xl bg-surface-container-low border border-outline-variant/10 hover:border-outline-variant/30 hover:shadow-sm transition-all cursor-pointer"
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                {canModifySchedule(s.id) && <button onClick={(e) => { e.stopPropagation(); handleDelete(s.id); }} disabled={actionId === s.id}
                      ...
                      return (
                        <tr key={s.id} className="hover:bg-surface-container-low/60 transition-colors group">
                          ...
                          <td className="px-6 py-4 text-right">
                            {canModifySchedule(s.id) && <button onClick={() => handleDelete(s.id)} disabled={actionId === s.id}
                  ...
                  <div>
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                      {activeCreateIntegrations.map((i) => {
                        ...
                        const isUnavailable = isTikTokUnavailable || isBrandMismatch || !canUseChannel(i.id);
                        return (
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select value={editForm.integrationId} onChange={(e) => setEditForm((f) => ({ ...f, integrationId: e.target.value }))}
                      ...
                      {integrations.filter((i) => i.isActive && canEditChannel(i.id)).map((i) => (
        ...
        {toast && (
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-3 bg-inverse-surface text-inverse-on-surface px-5 py-3 rounded-xl shadow-2xl animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            {toast === "Schedule deleted" && deletedItem ? (
              <button disabled={!!actionId} onClick={handleUndoDelete}
