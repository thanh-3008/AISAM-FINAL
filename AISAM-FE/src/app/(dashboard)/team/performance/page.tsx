...
export default function MemberPerformancePage() {
  ...
  return <main className="mx-auto w-full max-w-[1600px] space-y-6 px-4 py-6 text-on-surface sm:px-8 lg:py-8">
    ...
    <section className="grid grid-cols-1 items-end gap-4 rounded-2xl border border-outline-variant/40 bg-surface p-5 shadow-sm sm:grid-cols-2 xl:grid-cols-6 [&_label]:text-xs [&_label]:font-semibold [&_label]:text-on-surface-variant">
      ...
      <label>Team<select aria-label="Team" value={teamId} onChange={e => { setTeam(e.target.value); setBrand(""); setMember(""); setPage(1); }} className="mt-2 block w-full rounded-xl border border-outline-variant/50 bg-surface-container-low px-3 py-3 text-sm font-normal">
      ...
      <label>Member<select aria-label="Member" value={memberId} onChange={e => { setMember(e.target.value); setPage(1); }} className="mt-2 block w-full rounded-xl border border-outline-variant/50 bg-surface-container-low px-3 py-3 text-sm font-normal"><option value="">All Team members</option>{options.members.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
    ...
    {loading && <div role="status" className="rounded-2xl border border-outline-variant/40 bg-surface p-12 text-center text-sm text-on-surface-variant"><span className="mx-auto mb-4 block h-7 w-7 animate-spin rounded-full border-2 border-primary/20 border-t-primary" />Loading Team performance…</div>}
    ...
    {result && !result.teamSelectionRequired && <>
      <section aria-label="Performance overview" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {[
          ...
          ["Published targets", totals.creatorPosts, "send", "bg-emerald-50 text-emerald-600"],
      ...
      {result.items.length === 0 ? <div className="rounded-2xl border border-dashed border-outline-variant bg-surface px-6 py-16 text-center"><span className="material-symbols-outlined mb-3 !text-4xl text-primary/50">group</span><h2 className="font-semibold">No matching Team activity</h2><p className="mt-2 text-sm text-on-surface-variant">Try another Team, member or date range.</p></div> : <>
        {tab === "creation" && <DataTable headers={["Member", "Team role", "Contents created", "Published targets from own content", "Approval rate"]} rows={result.items.map(row => {
        ...
        {tab === "publishing" && <DataTable headers={["Member", "Team role", "Posts published", "Pending", "Completed", "Failed", "On time", "Failure rate"]} rows={result.items.map(row => {
