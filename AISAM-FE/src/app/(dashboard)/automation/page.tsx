...
export default function AutomationPage() {
  ...
  const [editForm, setEditForm] = useState({ brandId: "", teamId: "", productId: "", topic: "", platform: "facebook", contentType: "Text", objective: "", tone: "", cta: "", notes: "", scheduledAt: "" });
  ...
  const canReviewItem = (item: AutomationItem) => !rbac || rbac.workspaceRole !== "Member" || rbac.scopes.some(s => s.teamId === item.teamId && s.brandId === item.brandId && s.role === "Manager");
  ...
  const handleApprove = async (itemId?: string) => {
    if (!selected) return;
    if (selected.items.filter(i => itemId ? i.id === itemId : i.status === "AwaitingApproval").some(i => !canReviewItem(i))) { setMessage("Bạn không có quyền duyệt mọi Team trong lựa chọn này."); return; }
    setWorking(true); setMessage("");
    try {
      if (itemId) {
  ...
  const handleReject = async (itemId: string) => {
    if (!selected) return;
    if (!selected.items.some(i => i.id === itemId && canReviewItem(i))) { setMessage("Bạn không có quyền duyệt nội dung của Team này."); return; }
    setWorking(true); setMessage("");
  ...
  const openItemEditor = async (item: AutomationItem) => {
    ...
    setEditing(item);
    setEditForm({ brandId: item.brandId || "", teamId: item.teamId || "", productId: item.productId || "", topic: item.topic, platform: item.platform, contentType: item.contentType, objective: item.objective || "", tone: item.tone || "", cta: item.cta || "", notes: item.notes || "", scheduledAt: localValue });
    const [brandData, productData] = await Promise.all([fetchBrands(), item.brandId ? fetchProducts(item.brandId) : Promise.resolve([])]);
    setBrands(brandData); setProducts(productData);
  };

  const handleEditBrand = async (brandId: string) => {
    setEditForm((value) => ({ ...value, brandId, teamId: "", productId: "" }));
    setProducts(await fetchProducts(brandId));
  };

  const handleSaveItem = async () => {
    if (!selected || !editing) return;
    if ((rbac && !editForm.teamId) || !editForm.brandId || !editForm.topic.trim() || !editForm.scheduledAt) { setMessage("Brand, topic, date and time are required."); return; }
    setWorking(true); setMessage("");
    try {
      const plan = await updateAutomationItem(selected.id, editing.id, { ...editForm, teamId: rbac ? editForm.teamId : undefined, productId: editForm.productId || undefined, scheduledAt: new Date(editForm.scheduledAt).toISOString() });
  ...
  const downloadTemplate = () => {
    const csv = [
      `${rbac ? "TeamId," : ""}Brand,Product,Topic,Objective,Platforms,ContentType,Tone,CTA,Notes,Date,Time`,
      (rbac ? ',' : '') + 'Brand Name,Product Name,"New product introduction",Awareness,"Facebook|Instagram",Image,Youthful,"View now","Use brand colors",10/08/2026,09:00',
      (rbac ? ',' : '') + 'Brand Name,Product Name,"Tutorial video",Engagement,"Facebook|Instagram|TikTok",Video,Friendly,"Follow now","Vertical video",12/08/2026,19:30',
    ].join("\n");
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="p-5 space-y-4"><div className="grid grid-cols-3 gap-3"><div className="p-3 rounded-xl bg-emerald-500/10"><p className="text-lg font-bold text-emerald-700">{selected.validItems}</p><p className="text-label-xs">Valid</p></div><div className="p-3 rounded-xl bg-red-500/10"><p className="text-lg font-bold text-red-700">{selected.failedItems}</p><p className="text-label-xs">Needs Fix</p></div><div className="p-3 rounded-xl bg-primary/10"><p className="text-lg font-bold text-primary">{selected.estimatedCredits}</p><p className="text-label-xs">Est. Credits</p></div></div>
          ...
          {selected.items.map((item) => {
            const formatError = (err: string) => {
              if (!err) return err;
              if (err.includes("401") || err.includes("Unauthenticated")) return "API key không hợp lệ hoặc đã hết hạn (401).";
              if (err.includes("429") || err.includes("Too Many Attempts")) return "Vượt quá giới hạn gọi API (429). Thử lại sau.";
              if (err.startsWith("HTTP")) {
                try {
                  const jsonPart = err.substring(err.indexOf("{"));
      ...
      {editing && <div className="fixed inset-0 z-[100] flex items-center justify-center p-4"><div className="absolute inset-0 bg-black/60" onClick={() => setEditing(null)} /><div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-surface-container-lowest rounded-2xl p-6 shadow-2xl space-y-4">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <label><span className="text-label-2xs text-outline uppercase font-bold tracking-widest">Brand *</span><select disabled={!!editing.contentId} value={editForm.brandId} onChange={(event) => handleEditBrand(event.target.value)} className="mt-1.5 w-full px-3 py-3 rounded-xl bg-surface-container border border-outline-variant/20"><option value="">Select brand</option>{brands.map((brand) => <option key={brand.id} value={brand.id}>{brand.name}</option>)}</select></label>
        ...
        <TeamScopeSelect brandId={editForm.brandId} value={editForm.teamId} disabled={!!editing.contentId || working} onChange={teamId => setEditForm(value => ({ ...value, teamId }))} />
