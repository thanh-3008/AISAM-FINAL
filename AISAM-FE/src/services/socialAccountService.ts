...
export async function getFacebookAuthUrl(): Promise<AuthUrlResponse> {
  try {
    ...
    if (res?.data?.authUrl) return res.data;
  } catch {
    // fallback
  }
  return {
...
export async function handleFacebookCallback(data?: { code?: string; state?: string }): Promise<SocialAccount> {
  if (data?.code && data.state) {
    try {
      ...
      if (res?.data) return mapSocialAccount(res.data);
    } catch {
      // fallback
    }
  }
...
export async function linkTargets(
  accountId: string,
  targetIds: string[] | { targetIds: string[]; brandId?: string; provider?: SocialPlatform }
): Promise<SocialAccount> {
  const request = Array.isArray(targetIds)
    ? { targetIds, provider: "facebook" as SocialPlatform }
    : { provider: "facebook" as SocialPlatform, ...targetIds };
  ...
  const newTargets: SocialTarget[] = request.targetIds.map((id, i) => {
...
export async function deleteIntegration(integrationId: string): Promise<boolean> {
  try {
    const res: GenericResponse<boolean> = await apiClient(`/social/integrations/${integrationId}`, {
      method: "DELETE",
    });
