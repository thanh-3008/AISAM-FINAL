...
**25 quyết định, WorkspaceTypeEnum, Credits, Post Quota, Member Limit, Feature Matrix và Permission Matrix đã được lưu. Implementation đang ở Phase 9; Task 9.9 đã hoàn thành chuyển Subscription, Payment API và PayOS flow sang Active Workspace. Checkout/history/current được cô lập theo Workspace; payment thành công hỗ trợ renewal 30 ngày, idempotent webhook, đồng bộ `SubscriptionExpiredAt`, Business Plus Member Limit 10 và Business Premium Member Limit 50. Migration `20260611092549_AddWorkspacePaymentSubscriptionOwnership` đã có trong database và toàn bộ regression pass 198/198 tests. Dữ liệu cũ chưa được backfill để tránh suy diễn sai ownership; quota usage vẫn Profile-based và cộng Credits khi renewal chờ Credit Wallet ở Task 9.10. Task tiếp theo là 9.10 Credit Wallet, Credit Usage và Maximum Balance.**
